 ...
 * @wordpress-plugin
 * Plugin Name:       interactive-geo-maps   free   
...
function rdata_add_menu_page()
{
   ...
   add_menu_page('submit from data', 'interactive geo map free ', 'manage_options', 'submit-from-data', 'rdata_add_new_maps', '', 101);
   ...
   add_submenu_page('submit-from-data', 'contact', 'contact us', 'manage_options', 'submit-from-data-show', 'rdata_add_show_data', 101);
}
...
function ikr_endueue_scriptIn_parent () {
   ...
   wp_localize_script('ikr_interactive' , 'get_url', array(
      ...
      'tooltipForm'=> 'ikr_rdata_edit_all_tooltips'
  ) );}
}
